 ...
 * 【分布式事务】使用 @Transactional 保证状态更新的原子性
...
public class OrderStateMachineServiceImpl implements IOrderStateMachineService {
    ...
    public boolean paySuccess(String orderNo) {
        ...
        if (!order.canPay()) {
            log.warn("【状态机】订单状态不允许支付，orderNo={}, 当前状态={}", 
                    orderNo, order.getState());
            return false;
        }
    ...
    private void syncDBStockForPaySuccess(OrderEntity order) {
        ...
        for (OrderItemEntity item : order.getItems()) {
            ...
            if (productId == null || quantity == null) {
                log.warn("【库存同步】订单项数据异常，跳过，orderNo={}, item={}", 
                        order.getOrderNo(), item);
                continue;
            }

            boolean success = stockGateway.syncDBStockDeduct(productId, quantity);
            if (!success) {
                log.warn("【库存同步】DB库存扣减失败，productId={}, quantity={}, orderNo={}", 
                        productId, quantity, order.getOrderNo());
    ...
    public boolean deliver(String orderNo) {
        ...
        if (!order.canDeliver()) {
            log.warn("【状态机】订单状态不允许发货，orderNo={}, 当前状态={}", 
                    orderNo, order.getState());
            return false;
        }
    ...
    public boolean complete(String orderNo) {
        ...
        if (!order.canComplete()) {
            log.warn("【状态机】订单状态不允许完成，orderNo={}, 当前状态={}", 
                    orderNo, order.getState());
            return false;
        }
    ...
    public boolean cancel(String orderNo) {
        ...
        if (!order.canCancel()) {
            log.warn("【状态机】订单状态不允许取消，orderNo={}, 当前状态={}", 
                    orderNo, order.getState());
            return false;
        }
    ...
    private void restoreStockForCancel(OrderEntity order, boolean isPaid) {
        ...
        for (OrderItemEntity item : order.getItems()) {
            ...
            log.info("【库存恢复】Redis库存已恢复，productId={}, quantity={}, orderNo={}", 
                    productId, quantity, order.getOrderNo());
            ...
            if (isPaid) {
                ...
                if (dbRestoreSuccess) {
                    log.info("【库存恢复】MySQL库存已恢复，productId={}, quantity={}, orderNo={}", 
                            productId, quantity, order.getOrderNo());
                } else {
                    log.warn("【库存恢复】MySQL库存恢复失败，productId={}, quantity={}, orderNo={}", 
                            productId, quantity, order.getOrderNo());
                }
            }
        }
