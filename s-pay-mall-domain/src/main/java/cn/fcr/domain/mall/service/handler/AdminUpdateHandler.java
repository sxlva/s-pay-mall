...
public class AdminUpdateHandler implements StockChangeHandler {
    ...
    public long handle(Long productId, StockChangeMsgDTO msg) {
        ...
        log.info("【管理员更新库存策略】开始处理，productId=" + productId + ", messageId=" + messageId + ", businessNo=" + businessNo + ", newStock=" + newStock);
        ...
        if (!acquired) {
            ...
            log.info("【管理员更新库存策略】幂等性检查跳过，消息已处理或正在处理。productId=" + productId + ", businessNo=" + businessNo + ", currentStock=" + currentStock);
            return currentStock;
        }

        try {
            // 分支 A: 获取锁成功，执行更新逻辑
            log.info("【管理员更新库存策略】获取幂等锁成功，开始执行更新逻辑。productId=" + productId + ", businessNo=" + businessNo + ", newStock=" + newStock);

            // 直接设置 Redis 库存（全量更新）
            long syncedStock = stockGateway.setStock(productId, newStock);

            log.info("【管理员更新库存策略】处理成功，productId=" + productId + ", businessNo=" + businessNo + ", syncedStock=" + syncedStock);
            ...
            // 异常处理：释放幂等锁，允许后续重试消费
            log.error("【管理员更新库存策略】处理异常，释放幂等锁。productId=" + productId + ", businessNo=" + businessNo + ", error=" + e.getMessage());
